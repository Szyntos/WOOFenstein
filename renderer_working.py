...
class Renderer:
    def __init__(self, game_map, width3d, fov, ray_count):
        ...
        self.scales = [math.cos(a * math.pi / 180)/4 for a in self.angles]
    ...
    def draw_rays(self, x0, y0, orient):
        ...
        circle = [[r * math.sin((orient + self.angles[abs(i)] * ((i >= 0) - (i < 0)) + 180) * math.pi / 180) + x0,
                  ...
                  for i in range(-len(self.angles) + 1, len(self.angles))]
    ...
    def render(self):
        # print(self.rays)
        const = self.game_map.height**2
        scales = [self.scales[abs(i)] for i in range(-len(self.scales) + 1, len(self.scales))]
        self.rays = [[self.rays[i][j] * scales[i] for j in range(len(self.rays[i]))] for i in range(len(scales))]
        ray_count = len(self.rays)
        dw = self.width3d / ray_count
